...
class LatWeightedSpectralLossCrop(nn.Module):
    def __init__(
        self,
        ds: xr.Dataset,
    ...
    def forward(self, prediction, target, h_slice=(64, 85)) -> torch.Tensor:
        ...
        numerator = torch.sum(self.lat_weights[ :,:,h_slice[0]:h_slice[1],:] * diff, dim=(-2, -1))
        denominator = torch.sum(self.lat_weights[ :,:,h_slice[0]:h_slice[1],:])
