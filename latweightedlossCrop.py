...
class LatWeightedLossCrop(nn.Module):
    def __init__(self, ds: xr.Dataset, p: int = 2, device: str = "cpu", pool = 'mean'):
        super().__init__()
        self.p = p
        self.pool = pool
    ...
    def forward(self, prediction, target, h_slice = (64, 85)) -> torch.Tensor:
        diff = (target - prediction) ** self.p
        numerator = torch.sum(self.weights[ :,:,h_slice[0]:h_slice[1],:] * diff, dim=(-2, -1))
        denominator = torch.sum(self.weights[ :,:,h_slice[0]:h_slice[1],:] * prediction**self.p, dim=(-2,-1) )
